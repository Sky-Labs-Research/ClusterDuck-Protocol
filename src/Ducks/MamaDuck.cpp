...
void MamaDuck::cleanupStaleReassemblyBuffers()
{
  uint32_t now = millis();
 ...
 */
void MamaDuck::processIncomingFragment(const MavlinkFragmentPacket *incoming_packet)
{
...
void MamaDuck::handleMavlinkPacket(const CdpPacket &packet)
{
  // safety check to prevent reading out of bounds
  if (packet.data.size() < sizeof(MavlinkFragmentPacket))
  {
    // Or at least the size of the header if payloads can be variable
    Serial.println("Error: Received packet is too small.");
    return;
  }

  // pointer to the vector's raw data and cast it
